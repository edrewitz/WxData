"""
This file hosts the Google Weather Next 2 API Interface from Open-Meteo API
...
def weather_next_2_all_members(latitude,
            longitude,
            days=7,
            ...
            proxies=None,
            to_csv=False,
            path=f"Open Meteo Data/Google/Weather Next 2",
            filename=f"Weather_Next_2_All_Members.csv"):
    
    """
    This function retrieves Google Weather Next 2 time series forecast from the Open-Meteo API for a given point of latitude/longitude.
    ...
    A Pandas.DataFrame of the Google Weather Next 2 time series forecast for a given point of latitude/longitude. 
    """
    ...
    if proxies == None:
        response = _requests.get(f"https://ensemble-api.open-meteo.com/v1/ensemble?"
                             ...
                             f"&hourly={','.join(variables)}&models=google_weathernext2_ensemble"
...
def weather_next_2_ensemble_mean(latitude,
            longitude,
            days=7,
            temperature_units='fahrenheit',
            wind_speed_units='mph',
            ...
            to_csv=False,
            path=f"Open Meteo Data/Google/Weather Next 2",
            filename=f"Weather_Next_2_Ensemble_Mean.csv"):
    ...
    if proxies == None:
        response = _requests.get(f"https://ensemble-api.open-meteo.com/v1/ensemble?"
                             f"latitude={latitude}&longitude={longitude}"
                             f"&hourly={','.join(variables)}&models=google_weathernext2_ensemble_mean"
                             f"&forecast_days={days}&wind_speed_unit={wind_speed_units}"
                             f"&precipitation_unit={precipitation_units}&temperature_unit={temperature_units}")
