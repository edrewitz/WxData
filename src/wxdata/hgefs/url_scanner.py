"""
This file hosts the functions that act as URL scanners for the HGEFS.
...
def hgefs_url_scanner(final_forecast_hour,
                            proxies,
                            cat,
                            type_of_level):
    
    """
    This function is the URL scanner for the HGEFS Pressure Parameters.
    
    This function returns the URL and filename of the latest available HGEFS Pressure/Surface data on the NCEP NOMADS Server. 
    ...
    today_18z_url = f"https://nomads.ncep.noaa.gov/pub/data/nccf/com/hgefs/prod/hgefs.{now.strftime('%Y%m%d')}/18/ensstat/products/atmos/grib2/"
    today_12z_url = f"https://nomads.ncep.noaa.gov/pub/data/nccf/com/hgefs/prod/hgefs.{now.strftime('%Y%m%d')}/12/ensstat/products/atmos/grib2/"
    today_06z_url = f"https://nomads.ncep.noaa.gov/pub/data/nccf/com/hgefs/prod/hgefs.{now.strftime('%Y%m%d')}/06/ensstat/products/atmos/grib2/"
    today_00z_url = f"https://nomads.ncep.noaa.gov/pub/data/nccf/com/hgefs/prod/hgefs.{now.strftime('%Y%m%d')}/00/ensstat/products/atmos/grib2/"
    
    yesterday_18z_url = f"https://nomads.ncep.noaa.gov/pub/data/nccf/com/hgefs/prod/hgefs.{yd.strftime('%Y%m%d')}/18/ensstat/products/atmos/grib2/"
    yesterday_12z_url = f"https://nomads.ncep.noaa.gov/pub/data/nccf/com/hgefs/prod/hgefs.{yd.strftime('%Y%m%d')}/12/ensstat/products/atmos/grib2/"
    yesterday_06z_url = f"https://nomads.ncep.noaa.gov/pub/data/nccf/com/hgefs/prod/hgefs.{yd.strftime('%Y%m%d')}/06/ensstat/products/atmos/grib2/"
    yesterday_00z_url = f"https://nomads.ncep.noaa.gov/pub/data/nccf/com/hgefs/prod/hgefs.{yd.strftime('%Y%m%d')}/00/ensstat/products/atmos/grib2/"
    
    file_18z = f"hgefs.t18z.{level}.{cat}.f{final_forecast_hour}.grib2"
    file_12z = f"hgefs.t12z.{level}.{cat}.f{final_forecast_hour}.grib2"
    file_06z = f"hgefs.t06z.{level}.{cat}.f{final_forecast_hour}.grib2"
    file_00z = f"hgefs.t00z.{level}.{cat}.f{final_forecast_hour}.grib2"
    ...
    for response, url in zip(responses, urls):
        if response.status_code == 200:
            ...
            if run < 10:
                file = f"hgefs.t0{run}z.{level}.{cat}.f{final_forecast_hour}.grib2"
            else:
                file = f"hgefs.t{run}z.{level}.{cat}.f{final_forecast_hour}.grib2"
            break   
