...
def hrdps_url_scanner(final_forecast_hour,
    ...
    """
    This function scans for the latest available RDPS data from https://dd.weather.gc.ca/
    ...
    url_18z_today = f"{PREFIX_TODAY_18Z}/{final_forecast_hour}/"
    url_12z_today = f"{PREFIX_TODAY_12Z}/{final_forecast_hour}/"
    url_06z_today = f"{PREFIX_TODAY_06Z}/{final_forecast_hour}/"
    url_00z_today = f"{PREFIX_TODAY_00Z}/{final_forecast_hour}/"
    url_18z_yesterday = f"{PREFIX_YESTERDAY_18Z}/{final_forecast_hour}/"
    url_12z_yesterday = f"{PREFIX_YESTERDAY_12Z}/{final_forecast_hour}/"
    url_06z_yesterday = f"{PREFIX_YESTERDAY_06Z}/{final_forecast_hour}/"
    ...
    if type_of_level == 'pressure':
        ...
        file_18z_today = f"{now.strftime('%Y%m%d')}T18Z_MSC_{parameter}_ISBL_{strlevel}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"    
        file_12z_today = f"{now.strftime('%Y%m%d')}T12Z_MSC_{parameter}_ISBL_{strlevel}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_06z_today = f"{now.strftime('%Y%m%d')}T06Z_MSC_{parameter}_ISBL_{strlevel}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_00z_today = f"{now.strftime('%Y%m%d')}T00Z_MSC_{parameter}_ISBL_{strlevel}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_18z_yesterday = f"{yd.strftime('%Y%m%d')}T18Z_MSC_{parameter}_ISBL_{strlevel}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_12z_yesterday = f"{yd.strftime('%Y%m%d')}T12Z_MSC_{parameter}_ISBL_{strlevel}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_06z_yesterday = f"{yd.strftime('%Y%m%d')}T06Z_MSC_{parameter}_ISBL_{strlevel}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_00z_yesterday = f"{yd.strftime('%Y%m%d')}T00Z_MSC_{parameter}_ISBL_{strlevel}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        ...
        file_18z_today = f"{now.strftime('%Y%m%d')}T18Z_MSC_{parameter}_ISBY{level_low}-{level_high}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_12z_today = f"{now.strftime('%Y%m%d')}T12Z_MSC_{parameter}_ISBY-{level_low}-{level_high}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_06z_today = f"{now.strftime('%Y%m%d')}T06Z_MSC_{parameter}_ISBY-{level_low}-{level_high}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_00z_today = f"{now.strftime('%Y%m%d')}T00Z_MSC_{parameter}_ISBY-{level_low}-{level_high}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_18z_yesterday = f"{yd.strftime('%Y%m%d')}T18Z_MSC_{parameter}_ISBY-{level_low}-{level_high}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_12z_yesterday = f"{yd.strftime('%Y%m%d')}T12Z_MSC_{parameter}_ISBY-{level_low}-{level_high}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_06z_yesterday = f"{yd.strftime('%Y%m%d')}T06Z_MSC_{parameter}_ISBY-{level_low}-{level_high}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_00z_yesterday = f"{yd.strftime('%Y%m%d')}T00Z_MSC_{parameter}_ISBY-{level_low}-{level_high}_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        ...
        level_low = levels[0]
        level_high = levels[1]
        file_18z_today = f"{now.strftime('%Y%m%d')}T18Z_MSC_{parameter}_DBS-{level_low}-{level_high}cm_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_12z_today = f"{now.strftime('%Y%m%d')}T12Z_MSC_{parameter}_DBS-{level_low}-{level_high}cm_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_06z_today = f"{now.strftime('%Y%m%d')}T06Z_MSC_{parameter}_DBS-{level_low}-{level_high}cm_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_00z_today = f"{now.strftime('%Y%m%d')}T00Z_MSC_{parameter}_DBS-{level_low}-{level_high}cm_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_18z_yesterday = f"{yd.strftime('%Y%m%d')}T18Z_MSC_{parameter}_DBS-{level_low}-{level_high}cm_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_12z_yesterday = f"{yd.strftime('%Y%m%d')}T12Z_MSC_{parameter}_DBS-{level_low}-{level_high}cm_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_06z_yesterday = f"{yd.strftime('%Y%m%d')}T06Z_MSC_{parameter}_DBS-{level_low}-{level_high}cm_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
        file_00z_yesterday = f"{yd.strftime('%Y%m%d')}T00Z_MSC_{parameter}_DBS-{level_low}-{level_high}cm_RLatLon0.0225_PT{final_forecast_hour}H.grib2"
    ...
    if proxies == None:
        ...
        try:
            r0 = requests.get(f"{url_18z_today}{file_18z_today}",
                            stream=True)
            
            r0.close()
            
            r1 = requests.get(f"{url_12z_today}{file_12z_today}",
                                    stream=True)
            
            r1.close()
            
            r2 = requests.get(f"{url_06z_today}{file_06z_today}",
                                    stream=True)
            
            r2.close()
            
            r3 = requests.get(f"{url_00z_today}{file_00z_today}",
                                            stream=True)
            
            r3.close()
            
            r4 = requests.get(f"{url_18z_yesterday}{file_18z_yesterday}",
                                                        stream=True)
                        
            r4.close()
            
            r5 = requests.get(f"{url_12z_yesterday}{file_12z_yesterday}",
                                                        stream=True)
                        
            r5.close()
            
            r6 = requests.get(f"{url_06z_yesterday}{file_06z_yesterday}",
                                                        stream=True)
                        
            r6.close()
            
            r7 = requests.get(f"{url_00z_yesterday}{file_00z_yesterday}",
                                                        stream=True)
    ...
    for i in range(0, int(final_forecast_hour) + step, step):
        ...
        if type_of_level == 'pressure':
            ...
            file = f"{date}T{run}Z_MSC_{parameter}_ISBL_{strlevel}_RLatLon0.0225_PT{hour}H.grib2"
            ...
            file = f"{date}T{run}Z_MSC_{parameter}_ISBY-{level_low}-{level_high}_RLatLon0.0225_PT{hour}H.grib2"
            ...
            file = f"{date}T{run}Z_MSC_{parameter}_DBS-{level_low}-{level_high}cm_RLatLon0.0225_PT{hour}H.grib2"
