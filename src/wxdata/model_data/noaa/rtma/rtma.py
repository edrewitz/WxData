...
from wxdata.model_data.noaa.rtma.file_funcs import(
     build_directory as _build_directory,
     clear_idx_files as _clear_idx_files
)

from wxdata.model_data.noaa.rtma.url_scanners import(
...
from wxdata.model_data.noaa.rtma.file_scanner import local_file_scanner as _local_file_scanner
...
def rtma(model='rtma', 
    ...
    **If the client is unable to connect to the server the user specified, it will rotate to the next server and try to 
    ...
    try:
        url, filename, _, run = _rtma_url_scanner(model, 
                                                cat,
                                                proxies,
                                                source)
    except Exception as e:
