...
def fgoals_f3_h(latitude,
    ...
    if proxies == None:
        response = _requests.get(f"https://climate-api.open-meteo.com/v1/climate?"
                             f"latitude={latitude}&longitude={longitude}"
                             f"&start_date={start_date}&end_date={end_date}"
                             f"&daily={','.join(variables)}&models=FGOALS_f3_H"
                             f"&wind_speed_unit={wind_speed_units}"
                             f"&precipitation_unit={precipitation_units}&temperature_unit={temperature_units}")
...
def hiram_sit_hr(latitude,
            ...
            proxies=None,
            to_csv=False,
            path=f"Open Meteo Data/Climate/HiRAM_SIT_HR",
            filename=f"HiRAM_SIT_HR.csv"):
    
    """
    This function retrieves daily HiRAM_SIT_HR climate data from the Open-Meteo API for a given point of latitude/longitude.
    ...
    A Pandas.DataFrame of the HiRAM_SIT_HR time series for point latitude/longitude. 
    """
    ...
    if proxies == None:
        response = _requests.get(f"https://climate-api.open-meteo.com/v1/climate?"
                             f"latitude={latitude}&longitude={longitude}"
                             f"&start_date={start_date}&end_date={end_date}"
                             f"&daily={','.join(variables)}&models=HiRAM_SIT_HR"
                             f"&wind_speed_unit={wind_speed_units}"
                             f"&precipitation_unit={precipitation_units}&temperature_unit={temperature_units}")
...
def mri_agcm3_2_s(latitude,
            ...
            proxies=None,
            to_csv=False,
            path=f"Open Meteo Data/Climate/MRI_AGCM3_2_S",
            filename=f"MRI_AGCM3_2_S.csv"):
    
    """
    This function retrieves daily MRI_AGCM3_2_S climate data from the Open-Meteo API for a given point of latitude/longitude.
    ...
    A Pandas.DataFrame of the MRI_AGCM3_2_S time series for point latitude/longitude. 
    """
    ...
    if proxies == None:
        response = _requests.get(f"https://climate-api.open-meteo.com/v1/climate?"
                             f"latitude={latitude}&longitude={longitude}"
                             f"&start_date={start_date}&end_date={end_date}"
                             f"&daily={','.join(variables)}&models=MRI_AGCM3_2_S"
                             f"&wind_speed_unit={wind_speed_units}"
                             f"&precipitation_unit={precipitation_units}&temperature_unit={temperature_units}")
...
def ec_earth3p_hr(latitude,
            ...
            proxies=None,
            to_csv=False,
            path=f"Open Meteo Data/Climate/EC_Earth3P_HR",
            filename=f"EC_Earth3P_HR.csv"):
    
    """
    This function retrieves daily EC_Earth3P_HR climate data from the Open-Meteo API for a given point of latitude/longitude.
    ...
    A Pandas.DataFrame of the EC_Earth3P_HR time series for point latitude/longitude. 
    """
    ...
    if proxies == None:
        response = _requests.get(f"https://climate-api.open-meteo.com/v1/climate?"
                             f"latitude={latitude}&longitude={longitude}"
                             f"&start_date={start_date}&end_date={end_date}"
                             f"&daily={','.join(variables)}&models=EC_Earth3P_HR"
                             f"&wind_speed_unit={wind_speed_units}"
                             f"&precipitation_unit={precipitation_units}&temperature_unit={temperature_units}")
...
def mpi_esm1_2_xr(latitude,
    ...
    if proxies == None:
        response = _requests.get(f"https://climate-api.open-meteo.com/v1/climate?"
                             f"latitude={latitude}&longitude={longitude}"
                             f"&start_date={start_date}&end_date={end_date}"
                             f"&daily={','.join(variables)}&models=MPI_ESM1_2_XR"
                             f"&wind_speed_unit={wind_speed_units}"
                             f"&precipitation_unit={precipitation_units}&temperature_unit={temperature_units}")
...
def nicam16_8s(latitude,
            ...
            proxies=None,
            to_csv=False,
            path=f"Open Meteo Data/Climate/NICAM16_8S",
            filename=f"NICAM16_8S.csv"):
    
    """
    This function retrieves daily NICAM16_8S climate data from the Open-Meteo API for a given point of latitude/longitude.
    ...
    A Pandas.DataFrame of the NICAM16_8S time series for point latitude/longitude. 
    """
    ...
    if proxies == None:
        response = _requests.get(f"https://climate-api.open-meteo.com/v1/climate?"
                             f"latitude={latitude}&longitude={longitude}"
                             f"&start_date={start_date}&end_date={end_date}"
                             f"&daily={','.join(variables)}&models=NICAM16_8S"
                             f"&wind_speed_unit={wind_speed_units}"
                             f"&precipitation_unit={precipitation_units}&temperature_unit={temperature_units}")
