...
def ifs_hourly_point_forecast(latitude,
            longitude,
            days=7,
...
def aifs_hourly_point_forecast(latitude,
            longitude,
            days=7,
            temperature_units='fahrenheit',
            wind_speed_units='mph',
...
def ifs_hourly_hres_point_forecast(latitude,
            longitude,
            days=7,
            temperature_units='fahrenheit',
            wind_speed_units='mph',
...
def ifs_hourly_ensemble_point_forecast(latitude,
            longitude,
            days=7,
            temperature_units='fahrenheit',
            wind_speed_units='mph',
            ...
            proxies=None,
            to_csv=False,
            path=f"Open Meteo Data/ECMWF/IFS Ensemble",
            filename=f"IFS_Ensemble_Data.csv"):
    
    """
    This function retrieves IFS Ensemble time series forecast from the Open-Meteo API for a given point of latitude/longitude.
    ...
    A Pandas.DataFrame of the IFS Ensemble time series forecast for a given point of latitude/longitude. 
    """
    ...
    if proxies == None:
        response = _requests.get(f"https://ensemble-api.open-meteo.com/v1/ensemble?"
                             f"latitude={latitude}&longitude={longitude}"
                             f"&hourly={','.join(variables)}&models=ecmwf_ifs025_ensemble"
                             f"&forecast_days={days}&wind_speed_unit={wind_speed_units}"
                             f"&precipitation_unit={precipitation_units}&temperature_unit={temperature_units}")
...
def aifs_hourly_ensemble_point_forecast(latitude,
            longitude,
            days=7,
            temperature_units='fahrenheit',
            wind_speed_units='mph',
            ...
            proxies=None,
            to_csv=False,
            path=f"Open Meteo Data/ECMWF/AIFS Ensemble",
            filename=f"AIFS_Ensemble_Data.csv"):
    
    """
    This function retrieves AIFS Ensemble time series forecast from the Open-Meteo API for a given point of latitude/longitude.
    ...
    A Pandas.DataFrame of the AIFS Ensemble time series forecast for a given point of latitude/longitude. 
    """
    ...
    if proxies == None:
        response = _requests.get(f"https://ensemble-api.open-meteo.com/v1/ensemble?"
                             f"latitude={latitude}&longitude={longitude}"
                             f"&hourly={','.join(variables)}&models=ecmwf_aifs025_ensemble"
                             f"&forecast_days={days}&wind_speed_unit={wind_speed_units}"
                             f"&precipitation_unit={precipitation_units}&temperature_unit={temperature_units}")
