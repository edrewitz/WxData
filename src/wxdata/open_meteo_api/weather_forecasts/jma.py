...
def jma_hourly_point_forecast(latitude,
            ...
            proxies=None,
            to_csv=False,
            path=f"Open Meteo Data/JMA/JMA",
            filename=f"JMA_Data.csv"):
    
    """
    This function retrieves Japan Meteorological Agency (JMA) time series forecast from the Open-Meteo API for a given point of latitude/longitude.
    ...
    if days > 11:
        print(f"The maximum number of days that can be retrieved is 11. Setting 'days' to 11.")
        days = 11
    else:
        pass
    ...
    if proxies == None:
        response = _requests.get(f"https://api.open-meteo.com/v1/forecast?"
                             ...
                             f"&hourly={','.join(variables)}&models=jma_seamless"
